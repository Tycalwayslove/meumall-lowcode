...
- 按 `docs/material-layering-architecture.md` 继续推进 Phase 1：在已建立内部 primitives 原型并已改造 `ActionButton`、`ImageBanner`、`SectionTitle`、`StickyActionBar`、`CouponBundle`、`ProductRankList` 的基础上，继续迁移 `BrandFeatureSection`、`LiveEntry`、`StoreExpertSection`、`FlashSaleList`、`ActivityRuleModal`、`CouponSection` 等剩余业务物料中的 Button/Image/Tag/Text/Price 重复实现；更多业务物料复用且 API 稳定后，再评估抽独立 npm 包。
