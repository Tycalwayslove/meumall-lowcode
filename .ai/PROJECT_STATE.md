...
- React/Vue `BasicForm` 提交时会采集子级基础控件当前值，payload 保留 `formId`、`childCount`，并新增 `fieldCount`、`values`、`fieldLabels`、`fieldTypes`；表单校验、远程提交、登录、权限和风控仍由宿主 action handler 或后续 Java/BFF 协议承担。
