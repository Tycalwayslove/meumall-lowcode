...
- 当前发布检查、交付摘要、版本差异、Schema 片段预览、模板摘要和模板预览派生已沉淀到 editor API，发布链路和自动保存已抽象出部分本地持久化状态，但仍使用 localStorage mock，尚未切换真实 Java HTTP client、服务端草稿自动保存、服务端发布校验、审批和审计。
...
| 2026-08-01 | `本提交` | 沉淀编辑器模板摘要和预览 API。 |
