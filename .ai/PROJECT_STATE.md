...
- 当前发布检查、交付摘要、H5 预览入口、工作区状态摘要、Schema 文件导入导出、本地草稿持久化、版本差异、Schema 片段预览、发布历史列表、发布历史筛选、发布操作反馈、回滚备注、物料详情摘要、物料详情字段、物料默认预览 schema、物料收藏/最近使用偏好、节点操作菜单/快捷键模型、模板摘要、模板预览派生、页面起步命令、页面设置模型、数据源配置模型、事件绑定模型和动作配置模型已沉淀到 editor API，发布链路和自动保存仍使用 localStorage mock，尚未切换真实 Java HTTP client、服务端草稿自动保存、服务端发布校验、审批和审计。
...
| 2026-08-01 | 本提交 | 沉淀 editor 节点操作模型 API。 |
