...
- 内部 runtime primitives 原型已覆盖现有主要 React/Vue H5 物料，并补齐 `BasicButton`、`BasicInput`、`BasicTextarea`、`BasicSelect`、`BasicRadioGroup`、`BasicStepper`、`BasicSwitch`、`BasicCheckbox`、`BasicText`、`DividerBlock`、`BasicImage`、`BasicTag`、`BasicCard`、`BasicCarousel`、`BasicVideo` 可拖拽通用物料、`ProductList` 商品列表物料、`Input`、`Select`、`RadioGroup`、`Stepper`、`Textarea`、`Switch`、`Checkbox` 表单 primitives、`Overlay`、`Modal` 弹层 primitives、`CountdownText` 倒计时文本 primitive、`Tabs` 标签切换 primitive、`Spacer` 间距 primitive、`Divider` 分割线 primitive、`NoticeBar` 公告条 primitive 和 `RichText` 富文本展示 primitive；`ActivityHero`、`SectionContainer` 等历史或简单物料仍可按真实复用收益继续整理；primitives 仍不是公开 npm API。
...
| 日期 | 提交 | 说明 |
| --- | --- | --- |
| 2026-08-01 | `本次提交` | 增强 `ProductList` 商品列表物料，复用内部 primitives 并补齐样式配置、示例和 smoke 校验。 |
