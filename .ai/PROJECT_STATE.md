...
- Vue3 editor shell 组件化：已新增 `apps/editor-playground/src/components/EditorWorkspaceStats.vue`，顶部工作区状态条只消费 `LowcodeEditorWorkspaceStat[]` 展示模型；组件化迁移说明写入 `docs/editor-vue-shell-components.md`，后续物料目录、结构树、画布工具条、属性面板和发布面板按该边界继续拆分。
...
- Vue3 编辑器物料目录 API 复用：左侧物料分类、关键词过滤、收藏/最近使用物料恢复和物料卡片摘要已复用 `@meumall/lowcode-editor` 的 material catalog API；物料卡片展示配置项、事件和数据槽数量，方便运营判断物料复杂度。
...
| 2026-08-01 | 本提交 | 拆分 Vue3 编辑器工作区状态条组件。 |
