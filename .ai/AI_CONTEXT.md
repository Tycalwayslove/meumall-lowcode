...
- Vue3 editor shell 组件化说明已写入 `docs/editor-vue-shell-components.md`。当前已拆 `EditorWorkspaceStats`、`EditorMaterialCatalog`、`EditorMaterialDetail`、`EditorOutlineTree`、`EditorCanvasToolbar` 和 `EditorSelectedNodeCard`；后续优先按属性字段编辑器/资源选择器、页面设置、发布面板的顺序继续拆，不新增未规划的 Vue editor npm 包。
