...
- Vue3 editor shell 组件化说明已写入 `docs/editor-vue-shell-components.md`。当前已拆 `EditorWorkspaceStats`，只通过 props 消费 `LowcodeEditorWorkspaceStat[]`；后续优先按物料目录、结构树、画布工具条、属性面板、发布面板的顺序继续拆，不新增未规划的 Vue editor npm 包。
