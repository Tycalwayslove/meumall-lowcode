...
- 基础组件与物料分层架构已沉淀到 `docs/material-layering-architecture.md`：Design Tokens、Runtime Primitives、Generic Materials、Business Materials 分层治理；当前策略是先在 `materials-*` 包内部做 primitives 原型，稳定后再抽 `@meumall/lowcode-design-tokens`、`@meumall/lowcode-primitives-react-h5`、`@meumall/lowcode-primitives-vue-h5`，不要过早拆包。
