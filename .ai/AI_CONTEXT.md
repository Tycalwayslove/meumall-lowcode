...
- Vue3 editor shell 组件化说明已写入 `docs/editor-vue-shell-components.md`。当前已拆 `EditorWorkspaceStats`、`EditorMaterialCatalog`、`EditorMaterialDetail`、`EditorOutlineTree`、`EditorCanvasToolbar`、`EditorSelectedNodeCard` 和 `EditorPropGroupsPanel`；后续优先按资源选择器主面板、页面设置、发布面板的顺序继续拆，不新增未规划的 Vue editor npm 包。
