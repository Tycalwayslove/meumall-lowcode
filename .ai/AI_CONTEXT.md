...
- Vue3 编辑器 playground 画布已支持已有节点拖拽移动：设计模式下节点 wrapper 可拖动，支持移动到目标前后、移入 `SectionContainer`、移动到根节点末尾，并规避拖到自己或自己后代；物料插入、单节点移动和同父级成组移动的目标派生已复用 editor canvas drop target API。
