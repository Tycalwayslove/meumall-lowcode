...
- Vue3 编辑器属性字段分组已拆为 `apps/editor-playground/src/components/EditorPropGroupsPanel.vue`：组件接收属性分组、当前节点 props、组件名、折叠状态、列表项拖拽状态、列表项素材选择状态、素材筛选状态、素材列表和事件绑定模型，负责渲染属性字段控件、数组列表编辑器、列表项图片素材选择 UI 和事件绑定列表，并通过 emits 抛出属性更新、列表项操作、素材筛选/应用、商品快捷操作和事件绑定；真实 `replaceNodeProps`、节点 events 写回、素材查询、素材应用、商品 dataSource 绑定、权限、审计和服务端保存仍由 playground 或未来管理台 shell 持有。
- Vue3 编辑器资源选择器主面板已拆为 `apps/editor-playground/src/components/EditorResourcePanels.vue`：组件接收图片属性选项、素材/商品/优惠券/门店达人筛选状态、资源列表、选中资源 id、选中数量和当前节点 dataBinding 状态，负责渲染图片素材库、商品选择器、优惠券库和门店/达人库，并通过 emits 抛出筛选更新、资源勾选、应用资源、绑定数据源、清空和示例商品操作；真实 Resource Library Client 查询、选择状态维护、静态 props 写回、dataSource 写回、权限、审计和服务端保存仍由 playground 或未来管理台 shell 持有。
...
- Vue3 editor shell 组件化说明已写入 `docs/editor-vue-shell-components.md`。当前已拆 `EditorWorkspaceStats`、`EditorMaterialCatalog`、`EditorMaterialDetail`、`EditorOutlineTree`、`EditorCanvasToolbar`、`EditorSelectedNodeCard`、`EditorPropGroupsPanel` 和 `EditorResourcePanels`；后续优先按页面设置、发布面板的顺序继续拆，不新增未规划的 Vue editor npm 包。
