...
import type { LowcodeEditorMode } from "@meumall/lowcode-editor";
...
<template>
  <header class="topbar">
    ...
    <div class="toolbar" aria-label="编辑器工具栏">
      ...
      <button type="button" title="新建页面" class="page-start-trigger" @click="emit('open-page-start')">
        <Plus :size="17" />
        <span>新建</span>
      </button>
    ...
    <div class="toolbar compact" aria-label="历史与保存">
      ...
      <button type="button" title="保存草稿" @click="emit('save')">
        <Save :size="17" />
        <span>{{ props.dirty ? "保存草稿" : "已保存" }}</span>
      </button>
      <button type="button" title="导出当前页面 Schema" @click="emit('export-schema')">
        <Download :size="17" />
        <span>导出</span>
      </button>
      <button type="button" title="导入页面 Schema" @click="emit('import-schema')">
        <Upload :size="17" />
        <span>导入</span>
      </button>
      <button type="button" title="生成预览版本" @click="emit('create-preview')">
        <Eye :size="17" />
        <span>预览链接</span>
      </button>
      <button type="button" title="发布当前页面" @click="emit('publish')">
        <PanelRight :size="17" />
        <span>发布</span>
      </button>
      <button type="button" title="打开已发布 H5" @click="emit('open-runtime')">
        <MonitorSmartphone :size="17" />
        <span>打开 H5</span>
      </button>
      <button type="button" title="用 React H5 runtime 打开当前页面" @click="emit('open-react-runtime')">
        <MonitorSmartphone :size="17" />
        <span>React H5</span>
      </button>
    </div>
