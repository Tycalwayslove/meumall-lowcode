...
export function BasicLink({ props }: MaterialProps) {
  const variant = option<BasicLinkVariant>(props.variant, ["plain", "bar", "card"], "bar");
  const linkUrl = text(props.linkUrl);
  const subtitle = text(props.subtitle);
  const prefixText = text(props.prefixText);
  const disabled = boolean(props.disabled, false);
  const borderWidth = number(props.borderWidth, variant === "plain" ? 0 : 1);
  const onClick = typeof props.onClick === "function" ? props.onClick as BasicLinkClickHandler : undefined;
  const showSurface = variant !== "plain";

  return (
    <section
      className="mlc-material mlc-basic-link"
      style={{
    ...
    >
      <a
        className={`mlc-basic-link__inner mlc-basic-link__inner--${variant}`}
        href={!disabled && linkUrl ? linkUrl : undefined}
        target={boolean(props.targetBlank, false) ? "_blank" : undefined}
        rel={boolean(props.targetBlank, false) ? "noreferrer" : undefined}
        ...
        style={{
          ...
          minHeight: variant === "plain" ? undefined : 44,
          padding: showSurface ? number(props.padding, 12) : 0,
          color: text(props.textColor, disabled ? "#94a3b8" : "#111827"),
          textDecoration: boolean(props.underline, variant === "plain") ? "underline" : "none",
          border: showSurface && borderWidth > 0 ? `${borderWidth}px solid ${text(props.borderColor, "#e5e7eb")}` : undefined,
          borderRadius: showSurface ? number(props.radius, variant === "card" ? 12 : 10) : undefined,
          background: showSurface ? text(props.backgroundColor, "#ffffff") : "transparent",
          boxShadow: variant === "card" && boolean(props.shadow) ? "0 10px 24px rgba(15, 23, 42, 0.08)" : undefined,
      ...
      >
        {prefixText ? (
          <MlcTag
            tone="accent"
            radius={number(props.prefixRadius, 999)}
            className="mlc-basic-link__prefix"
            style={{
              color: text(props.prefixColor, "#0f766e"),
              background: text(props.prefixBackgroundColor, "rgba(15, 118, 110, 0.1)"),
            }}
          >
            {prefixText}
          </MlcTag>
        ) : null}
        <span className="mlc-basic-link__content" style={{ display: "grid", gap: subtitle ? 3 : 0, minWidth: 0 }}>
          <MlcText as="strong" size={number(props.fontSize, 14)} weight={number(props.fontWeight, 800)} lineHeight={1.45} style={{ color: text(props.textColor, disabled ? "#94a3b8" : "#111827") }}>
            {text(props.text, "基础链接")}
          </MlcText>
          {subtitle ? (
            <MlcText as="span" size={12} lineHeight={1.45} style={{ color: text(props.subtitleColor, "#64748b") }}>
              {subtitle}
            </MlcText>
          ) : null}
        </span>
        {boolean(props.showArrow, true) ? (
          <span
            className="mlc-basic-link__arrow"
            aria-hidden="true"
            style={{
              color: text(props.arrowColor, "#94a3b8"),
              fontSize: number(props.arrowSize, 20),
              fontWeight: 900,
              lineHeight: 1,
            }}
          >
            {text(props.arrowText, "›")}
          </span>
        ) : null}
      </a>
    </section>
    ...
    <BasicFormValidationContext.Provider value={validationContext}>
      ...
      <form
        ...
        style={{
          ...
          boxShadow: boolean(props.shadow, true) ? "0 10px 28px rgba(15, 23, 42, 0.08)" : undefined,
        }}
      >
        {title || description ? (
          <div className="mlc-basic-form__header" style={{ display: "grid", gap: 4 }}>
            {title ? (
            ...
            {description ? (
              <MlcText as="p" size={13} lineHeight={1.6} style={{ color: text(props.descriptionColor, "#64748b") }}>
                {description}
              </MlcText>
            ) : null}
          </div>
        ...
        {hasChildren ? (
          <div className="mlc-basic-form__fields" style={{ display: "grid", gap }}>
            {children}
          </div>
        ) : (
          <MlcText
            ...
            tone="muted"
            lineHeight={1.5}
            className="mlc-basic-form__empty"
            style={{
              padding: 14,
          ...
          >
            {text(props.emptyText, "向表单中添加输入物料")}
          </MlcText>
        )}
        {validationMessages.length ? (
          <div
            className="mlc-basic-form__errors"
            role="alert"
            style={{
              display: "grid",
              gap: 4,
              padding: "10px 12px",
              border: `1px solid ${text(props.errorBorderColor, "#fecaca")}`,
              borderRadius: 8,
              color: text(props.errorColor, "#b91c1c"),
              background: text(props.errorBackgroundColor, "#fef2f2"),
              fontSize: 12,
              lineHeight: 1.5,
            }}
          >
            <strong>{text(props.validationErrorText, "请完善必填项后再提交")}</strong>
        ...
        <MlcButton
          ...
          block
          size="lg"
          radius={number(props.buttonRadius, 10)}
          disabled={disabled}
          loading={loading}
          style={{
            borderColor: text(props.buttonColor, "#111827"),
            color: text(props.buttonTextColor, "#ffffff"),
            background: text(props.buttonColor, "#111827"),
          }}
        >
          {text(props.submitText, "提交")}
        </MlcButton>
        {submitted && boolean(props.showSuccessMessage, true) ? (
          <MlcText as="p" size={12} lineHeight={1.5} className="mlc-basic-form__success" style={{ color: text(props.successColor, "#0f766e"), textAlign: "center" }}>
            {text(props.successText, "已触发表单提交事件")}
          </MlcText>
        ) : null}
      </form>
      </section>
...
export function BasicList({ props }: MaterialProps) {
  const items = list(props.items).length ? list(props.items) : BASIC_LIST_FALLBACK_ITEMS;
  const title = text(props.title, "基础列表");
  const subtitle = text(props.subtitle);
  const marker = option<BasicListMarker>(props.marker, ["dot", "number", "badge", "none"], "dot");
  const gap = number(props.gap, 8);
  const borderWidth = number(props.borderWidth, 1);
  const onItemClick = typeof props.onItemClick === "function" ? props.onItemClick as BasicListItemClickHandler : undefined;

  function renderMarker(item: Record<string, unknown>, index: number) {
    if (marker === "none") return null;
    if (marker === "badge") {
      return (
        <MlcTag tone="accent" radius={number(props.markerRadius, 999)} className="mlc-basic-list__badge">
    ...
    return (
      <span
        className={`mlc-basic-list__marker mlc-basic-list__marker--${marker}`}
        aria-hidden="true"
        style={{
          display: "inline-flex",
          alignItems: "center",
          ...
          flex: "0 0 auto",
          width: marker === "number" ? 24 : 9,
          height: marker === "number" ? 24 : 9,
          borderRadius: 999,
          color: text(props.markerTextColor, "#ffffff"),
          background: text(props.markerColor, "#0f766e"),
          fontSize: 12,
          fontWeight: 900,
          lineHeight: 1,
          marginTop: marker === "number" ? 0 : 7,
        }}
      >
  ...
  return (
    <section
      className="mlc-material mlc-basic-list"
      style={{
        padding: `${number(props.paddingY, 14)}px 12px`,
        background: text(props.backgroundColor, "#f3f4f6"),
      }}
    >
      <div
        className="mlc-basic-list__card"
        style={{
          display: "grid",
          gap,
          padding: number(props.padding, 14),
      ...
      >
        {title || subtitle ? (
          <div className="mlc-basic-list__header" style={{ display: "grid", gap: 4 }}>
            {title ? (
              <MlcText as="strong" size={18} weight={900} style={{ color: text(props.titleColor, "#111827") }}>
                {title}
              </MlcText>
        ...
        <div className="mlc-basic-list__items" style={{ display: "grid", gap }}>
          {items.map((item, index) => {
            const clickable = Boolean(onItemClick);
            return (
              <div
                key={text(item.id, `${index}`)}
                className="mlc-basic-list__item"
              ...
              >
                ...
                <div style={{ display: "grid", gap: 4, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                    ...
                    {text(item.metaText) ? (
                      <MlcText size={12} lineHeight={1.4} style={{ flex: "0 0 auto", color: text(props.metaColor, "#64748b") }}>
                        {text(item.metaText)}
                      </MlcText>
                    ) : null}
                  </div>
...
export function BasicAccordion({ props, node }: MaterialProps) {
  const items = list(props.items).length ? list(props.items) : BASIC_ACCORDION_FALLBACK_ITEMS;
  const title = text(props.title, "基础折叠面板");
  const subtitle = text(props.subtitle);
  const mode = option<BasicAccordionMode>(props.mode, ["single", "multiple"], "single");
  const icon = option<BasicAccordionIcon>(props.icon, ["chevron", "plus", "none"], "chevron");
  const gap = number(props.gap, 8);
  const borderWidth = number(props.borderWidth, 1);
  const defaultOpenIds = stringList(props.defaultOpenIds);
  const firstItemId = items[0] ? text(items[0].id, "0") : "";
  const initialOpenIds = defaultOpenIds.length ? defaultOpenIds : boolean(props.defaultOpenFirst, true) && firstItemId ? [firstItemId] : [];
  const [openIds, setOpenIds] = React.useState<string[]>(initialOpenIds);
  const onItemToggle = typeof props.onItemToggle === "function" ? props.onItemToggle as BasicAccordionToggleHandler : undefined;

  React.useEffect(() => {
    setOpenIds(initialOpenIds);
  }, [props.defaultOpenFirst, props.defaultOpenIds, props.items, node.id]);
  ...
  function renderIcon(open: boolean) {
    ...
    return (
      <span
        className={`mlc-basic-accordion__icon mlc-basic-accordion__icon--${icon}${open ? " mlc-basic-accordion__icon--open" : ""}`}
        aria-hidden="true"
        style={{
          display: "inline-flex",
          alignItems: "center",
          ...
          height: 22,
          borderRadius: 999,
          color: text(props.iconColor, "#64748b"),
          background: text(props.iconBackgroundColor, "transparent"),
          fontSize: icon === "plus" ? 18 : 20,
          fontWeight: 900,
          lineHeight: 1,
  ...
  return (
    <section
      className="mlc-material mlc-basic-accordion"
      style={{
        padding: `${number(props.paddingY, 14)}px 12px`,
        background: text(props.backgroundColor, "#f3f4f6"),
      }}
    >
      <div
        className="mlc-basic-accordion__card"
        style={{
          display: "grid",
          gap,
          padding: number(props.padding, 14),
      ...
      >
        {title || subtitle ? (
          <div className="mlc-basic-accordion__header" style={{ display: "grid", gap: 4 }}>
            {title ? (
              <MlcText as="strong" size={18} weight={900} style={{ color: text(props.titleColor, "#111827") }}>
                {title}
              </MlcText>
        ...
        <div className="mlc-basic-accordion__items" style={{ display: "grid", gap }}>
          {items.map((item, index) => {
            const itemId = text(item.id, `${index}`);
            const open = openIds.includes(itemId);
            const contentId = `${node.id}-${itemId}-accordion-panel`;

            return (
              <article
                key={itemId}
                className={`mlc-basic-accordion__item${open ? " mlc-basic-accordion__item--open" : ""}`}
                style={{
                  overflow: "hidden",
                  borderRadius: number(props.itemRadius, 10),
                  border: `1px solid ${text(props.itemBorderColor, "#e5e7eb")}`,
              ...
              >
                <button
                  type="button"
                  className="mlc-basic-accordion__trigger"
                  aria-expanded={open}
                  aria-controls={contentId}
                  onClick={() => {
                    const nextOpen = !open;
                    setOpenIds((currentOpenIds) => {
                      if (!nextOpen) return currentOpenIds.filter((id) => id !== itemId);
                      if (mode === "single") return [itemId];
                      return currentOpenIds.includes(itemId) ? currentOpenIds : [...currentOpenIds, itemId];
                    });
                  ...
                  style={{
                    ...
                    padding: number(props.itemPadding, 12),
                    border: 0,
                    color: text(props.itemTitleColor, "#111827"),
                  ...
                  <span style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                    {text(item.badgeText) ? (
                      <MlcTag
                        ...
                        className="mlc-basic-accordion__badge"
                        style={{
                          color: text(props.badgeColor, "#0f766e"),
                ...
                {open ? (
                  <div
                    id={contentId}
                    className="mlc-basic-accordion__panel"
                    style={{
                      padding: `0 ${number(props.itemPadding, 12)}px ${number(props.itemPadding, 12)}px`,
                      color: text(props.contentColor, "#64748b"),
                    }}
                  >
                    <MlcText as="p" size={13} lineHeight={1.65} style={{ color: text(props.contentColor, "#64748b"), whiteSpace: "pre-line" }}>
...
export function BasicStateBlock({ props }: MaterialProps) {
  const state = option<BasicStateBlockState>(props.state, ["empty", "loading", "error", "success", "info"], "empty");
  const palette = BASIC_STATE_BLOCK_PALETTES[state];
  const actionText = text(props.actionText);
  const showAction = boolean(props.showAction, Boolean(actionText));
  const onActionClick = typeof props.onActionClick === "function" ? props.onActionClick as BasicStateBlockActionHandler : undefined;

  return (
    <section
      className={`mlc-material mlc-basic-state-block mlc-basic-state-block--${state}`}
      style={{
        padding: `${number(props.paddingY, 14)}px 12px`,
        background: text(props.wrapperBackgroundColor, "#f3f4f6"),
      }}
    >
      <MlcStateBlock
        state={state}
        title={text(props.title, "暂无内容")}
        description={text(props.description, "可以用它展示空态、加载态、错误态或成功反馈。")}
        iconText={text(props.iconText, palette.icon)}
        showIcon={boolean(props.showIcon, true)}
        align={option<"left" | "center">(props.align, ["left", "center"], "center")}
        backgroundColor={text(props.backgroundColor, palette.background)}
        borderColor={text(props.borderColor, palette.border)}
        titleColor={text(props.titleColor, palette.title)}
        descriptionColor={text(props.descriptionColor, palette.description)}
        accentColor={text(props.accentColor, palette.accent)}
        iconColor={text(props.iconColor, "#ffffff")}
        iconBackgroundColor={text(props.iconBackgroundColor, palette.accent)}
        radius={number(props.radius, 14)}
        borderWidth={number(props.borderWidth, 1)}
        padding={number(props.padding, 22)}
        gap={number(props.gap, 10)}
        minHeight={number(props.minHeight, 148)}
        action={showAction && actionText ? (
          <MlcButton
            ...
            style={{
              color: text(props.actionTextColor, "#ffffff"),
              background: text(props.actionBackgroundColor, palette.accent),
              borderColor: text(props.actionBackgroundColor, palette.accent),
            }}
          >
            {actionText}
          </MlcButton>
...
export function BasicProgress({ props }: MaterialProps) {
  const tone = option<BasicProgressTone>(props.tone, ["brand", "success", "warning", "danger", "neutral"], "brand");
  const palette = BASIC_PROGRESS_TONE_PALETTES[tone];
  const max = Math.max(1, number(props.max, 100));
  const value = Math.min(max, Math.max(0, number(props.value, 68)));
  const suffix = text(props.valueSuffix, "%");

  return (
    <section
      className={`mlc-material mlc-basic-progress mlc-basic-progress--${tone}`}
      style={{
        padding: `${number(props.paddingY, 14)}px 12px`,
        background: text(props.wrapperBackgroundColor, "#f3f4f6"),
      }}
    >
      <div
        className="mlc-basic-progress__card"
        style={{
          display: "grid",
          gap: number(props.gap, 10),
          padding: number(props.padding, 14),
          border: `${number(props.borderWidth, 1)}px solid ${text(props.borderColor, "#e5e7eb")}`,
          borderRadius: number(props.radius, 12),
          background: text(props.backgroundColor, "#ffffff"),
          boxShadow: boolean(props.shadow) ? "0 10px 24px rgba(15, 23, 42, 0.08)" : "none",
        }}
      >
        <MlcProgress
          value={value}
          max={max}
          label={text(props.title, "基础进度")}
          helperText={text(props.description, "用于展示静态进度、达成率或完成度。")}
          showValue={boolean(props.showValue, true)}
          valueText={`${value}${suffix}`}
          trackColor={text(props.trackColor, palette.track)}
          fillColor={text(props.fillColor, palette.fill)}
          labelColor={text(props.titleColor, palette.title)}
          helperColor={text(props.descriptionColor, palette.description)}
...
export function BasicMetricGrid({ props }: MaterialProps) {
  const tone = option<BasicMetricGridTone>(props.tone, ["brand", "success", "warning", "danger", "neutral"], "brand");
  const variant = option<BasicMetricGridVariant>(props.variant, ["card", "plain"], "card");
  const align = option<BasicInlineAlign>(props.align, ["left", "center", "right"], "center");
  const palette = BASIC_METRIC_TONE_PALETTES[tone];
  const columns = Math.min(3, Math.max(1, number(props.columns, 3)));
  const items = (list(props.items).length ? list(props.items) : BASIC_METRIC_GRID_FALLBACK_ITEMS).slice(0, 6);

  return (
    <section
      className={`mlc-material mlc-basic-metric-grid mlc-basic-metric-grid--${tone} mlc-basic-metric-grid--${variant}`}
      style={{
        padding: `${number(props.paddingY, 14)}px 12px`,
        background: text(props.wrapperBackgroundColor, "#f3f4f6"),
      }}
    >
      <div
        className="mlc-basic-metric-grid__card"
        style={{
          display: "grid",
      ...
      >
        {text(props.title) || text(props.description) ? (
          <header className="mlc-basic-metric-grid__header" style={{ display: "grid", gap: 4, textAlign: align }}>
            {text(props.title) ? (
              <MlcText as="strong" size={15} weight={800} style={{ color: text(props.titleColor, "#111827") }}>
                {text(props.title)}
              </MlcText>
            ) : null}
            {text(props.description) ? (
              <MlcText as="span" size={12} style={{ color: text(props.descriptionColor, "#64748b") }}>
                {text(props.description)}
              </MlcText>
            ) : null}
          </header>
        ...
        <div
          className="mlc-basic-metric-grid__items"
          style={{
            display: "grid",
            gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`,
            gap: number(props.itemGap, 8),
          }}
        >
          {items.map((item, index) => (
            <div
              key={text(item.id, `metric_${index}`)}
              className="mlc-basic-metric-grid__item"
              style={{
                minWidth: 0,
                padding: variant === "card" ? number(props.itemPadding, 12) : 0,
                border: variant === "card" ? `${number(props.itemBorderWidth, 1)}px solid ${text(props.itemBorderColor, palette.border)}` : "0 solid transparent",
                borderRadius: variant === "card" ? number(props.itemRadius, 10) : 0,
                background: variant === "card" ? text(props.itemBackgroundColor, palette.background) : "transparent",
              }}
            >
              <MlcMetric
                ...
                suffixColor={text(props.suffixColor, palette.helper)}
                helperColor={text(props.helperColor, palette.helper)}
                valueSize={number(props.valueSize, 26)}
                labelSize={number(props.labelSize, 12)}
                helperSize={number(props.helperSize, 11)}
                gap={number(props.metricGap, 4)}
              />
            </div>
          ))}
        </div>
...
export function BasicPrice({ props }: MaterialProps) {
  const align = option<BasicInlineAlign>(props.align, ["left", "center", "right"], "left");

  return (
    <section
      className="mlc-material mlc-basic-price"
      style={{
        padding: `${number(props.paddingY, 10)}px 16px`,
        background: text(props.wrapperBackgroundColor, "transparent"),
        textAlign: align,
    ...
    >
      <MlcPrice
        amountText={text(props.amountText, "99")}
        prefix={text(props.prefix, "¥")}
        suffix={text(props.suffix)}
        size={number(props.size, 24)}
        style={{ color: text(props.color, "#dc2626") }}
      />
    </section>
  );
}
