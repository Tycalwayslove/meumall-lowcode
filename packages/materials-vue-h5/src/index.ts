import { defineComponent, h, inject, onBeforeUnmount, provide, ref, watch, type CSSProperties, type PropType, type Ref } from "vue";
...
const BASIC_FORM_VALIDATION_CONTEXT_KEY = Symbol("BasicFormValidationContext");

type BasicFormValidationContextValue = {
  fieldErrors: Ref<Record<string, string>>;
  errorColor: Ref<string>;
...
function injectBasicFormValidation(): BasicFormValidationContextValue | null {
  return inject<BasicFormValidationContextValue | null>(BASIC_FORM_VALIDATION_CONTEXT_KEY, null);
}

function getBasicFormFieldError(validation: BasicFormValidationContextValue | null, node: LowcodeNode | undefined): string {
  return node ? validation?.fieldErrors.value[node.id] ?? "" : "";
}

function clearBasicFormFieldError(validation: BasicFormValidationContextValue | null, node: LowcodeNode | undefined): void {
  if (node) validation?.clearFieldError(node.id);
}
...
function createBasicFormFieldAttrs(baseClassName: string, errorMessage: string): Record<string, string | boolean | undefined> {
  return {
    class: createBasicFormFieldClassName(baseClassName, errorMessage),
...
function renderBasicFormFieldError(errorMessage: string, errorColor: string) {
  ...
  return h(
    MlcText,
    {
      as: "p",
      size: 12,
      class: "mlc-basic-form__field-error",
      style: {
        color: errorColor,
        fontWeight: 700,
...
export const BasicModal = defineComponent({
  ...
  props: materialPropOptions,
  setup(props) {
    const open = ref(boolean(props.props?.defaultOpen, false));

    watch(
      () => props.props?.defaultOpen,
      (nextValue) => {
        open.value = boolean(nextValue, false);
      },
    );

    return () => {
      const runtimeProps = props.props ?? {};
      const placement = option<BasicModalPlacement>(runtimeProps.placement, ["center", "bottom"], "bottom");
      const buttonColor = text(runtimeProps.buttonColor, "#111827");
      const buttonTextColor = text(runtimeProps.buttonTextColor, "#ffffff");
      const radius = number(runtimeProps.radius, 10);
      const contentPadding = number(runtimeProps.contentPadding, 16);
      const handler = runtimeProps.onOpen;

      return h(
        "section",
        {
          class: "mlc-material mlc-basic-modal",
          style: {
            display: "grid",
            gap: "8px",
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
        ...
        [
          h(
            ...
            () => text(runtimeProps.triggerText, "打开弹窗"),
          ),
          text(runtimeProps.summary)
            ? h(
                MlcText,
                {
                  as: "p",
          ...
          h(
            ...
            {
              open: open.value,
              title: text(runtimeProps.modalTitle, "基础弹窗"),
              closeLabel: text(runtimeProps.closeLabel, "关闭弹窗"),
              placement,
              closeOnBackdrop: boolean(runtimeProps.closeOnBackdrop, true),
              radius: placement === "bottom" ? `${number(runtimeProps.radius, 16)}px ${number(runtimeProps.radius, 16)}px 12px 12px` : number(runtimeProps.radius, 16),
              style: { background: text(runtimeProps.modalBackgroundColor, "#ffffff") } satisfies CSSProperties,
              bodyStyle: { padding: `0 ${contentPadding}px ${contentPadding}px` } satisfies CSSProperties,
              onClose: () => {
                open.value = false;
              },
            },
              ...
              default: () =>
                h(
                  MlcText,
                  {
                    ...
                    size: 14,
                    lineHeight: 1.7,
                    style: { color: text(runtimeProps.contentColor, "#374151"), whiteSpace: "pre-line" } satisfies CSSProperties,
                  },
                  () => text(runtimeProps.content, "这里可以配置通用说明、活动提示或运营引导内容。"),
                ),
              footer: () =>
                h(
                  "div",
                  { style: { padding: `0 ${contentPadding}px ${contentPadding}px` } satisfies CSSProperties },
                  h(
                    MlcButton,
                    {
                      block: true,
                      radius,
                      style: { borderColor: buttonColor, color: buttonTextColor, background: buttonColor } satisfies CSSProperties,
                      onClick: () => {
                        open.value = false;
                      },
                    },
                    () => text(runtimeProps.primaryText, "知道了"),
                  ),
                ),
            },
          ),
...
export const BasicForm = defineComponent({
  ...
  setup(props, { slots }) {
    ...
    const fieldErrorColor = ref(BASIC_FORM_DEFAULT_ERROR_COLOR);
    ...
    watch(
      () => props.props?.errorColor,
      (nextColor) => {
        fieldErrorColor.value = text(nextColor, BASIC_FORM_DEFAULT_ERROR_COLOR);
      },
      { immediate: true },
    );

    provide(BASIC_FORM_VALIDATION_CONTEXT_KEY, {
      fieldErrors,
      errorColor: fieldErrorColor,
      clearFieldError,
    } satisfies BasicFormValidationContextValue);
      ...
      return h(
        ...
        h(
          ...
          [
            ...
            Object.values(fieldErrors.value).length
              ? h(
                  "div",
                  {
                  ...
                  [
                    ...
                    ...Object.values(fieldErrors.value).map((message) => h("span", { key: message }, message)),
                  ],
                )
              : null,
            h(
...
export const BasicTimeline = defineComponent({
  name: "BasicTimeline",
  props: materialPropOptions,
  setup(props) {
    return () => {
      const runtimeProps = props.props ?? {};
      const items = list(runtimeProps.items).length ? list(runtimeProps.items) : BASIC_TIMELINE_FALLBACK_ITEMS;
      const title = text(runtimeProps.title, "基础时间线");
      const subtitle = text(runtimeProps.subtitle);
      const marker = option<BasicTimelineMarker>(runtimeProps.marker, ["dot", "number", "badge"], "dot");
      const showTime = boolean(runtimeProps.showTime, true);
      const showConnector = boolean(runtimeProps.showConnector, true);
      const gap = number(runtimeProps.gap, 12);
      const borderWidth = number(runtimeProps.borderWidth, 1);
      const onItemClick = typeof runtimeProps.onItemClick === "function" ? runtimeProps.onItemClick as BasicTimelineItemClickHandler : undefined;
      const markerTextColor = text(runtimeProps.markerTextColor, "#ffffff");
      const markerRadius = number(runtimeProps.markerRadius, 999);

      const itemStatus = (item: Record<string, unknown>) => option<BasicTimelineStatus>(item.status, ["done", "active", "pending"], "pending");

      const renderMarker = (item: Record<string, unknown>, index: number, status: BasicTimelineStatus) => {
        const palette = BASIC_TIMELINE_STATUS_PALETTES[status];
        const markerColor = text(runtimeProps.markerColor, palette.marker);
        ...
        if (marker === "badge") {
          return h(
            MlcTag,
            {
              tone: "accent",
              radius: markerRadius,
              class: `mlc-basic-timeline__badge mlc-basic-timeline__badge--${status}`,
              style: {
                color: markerColor,
                background: text(runtimeProps.badgeBackgroundColor, "rgba(15, 118, 110, 0.1)"),
              } satisfies CSSProperties,
        ...
        return h(
          "span",
          {
            class: `mlc-basic-timeline__marker mlc-basic-timeline__marker--${marker} mlc-basic-timeline__marker--${status}`,
            "aria-hidden": "true",
            style: {
              display: "inline-flex",
              alignItems: "center",
              justifyContent: "center",
              width: marker === "number" ? "26px" : "11px",
              height: marker === "number" ? "26px" : "11px",
              borderRadius: `${markerRadius}px`,
              border: marker === "dot" ? `3px solid ${text(runtimeProps.itemBackgroundColor, "#ffffff")}` : undefined,
      ...
      return h(
        "section",
        {
          class: "mlc-material mlc-basic-timeline",
          style: {
            padding: `${number(runtimeProps.paddingY, 14)}px 12px`,
            background: text(runtimeProps.backgroundColor, "#f3f4f6"),
          } satisfies CSSProperties,
        ...
        h(
          "div",
          {
            class: "mlc-basic-timeline__card",
            style: {
              display: "grid",
              gap: `${gap}px`,
              padding: `${number(runtimeProps.padding, 14)}px`,
          ...
          [
            title || subtitle
              ? h("div", { class: "mlc-basic-timeline__header", style: { display: "grid", gap: "4px" } }, [
                  title
                    ? h(MlcText, { as: "strong", size: 18, weight: 900, style: { color: text(runtimeProps.titleColor, "#111827") } }, () => title)
                    : null,
                  subtitle
            ...
            h(
              "ol",
              { class: "mlc-basic-timeline__items", style: { display: "grid", gap: `${gap}px`, padding: 0, margin: 0, listStyle: "none" } },
              items.map((item, index) => {
                ...
                const content = h(
                  "div",
                  {
                    class: "mlc-basic-timeline__content",
                    style: {
                      display: "grid",
                      gap: "5px",
                      minWidth: 0,
                  ...
                  [
                    showTime && text(item.timeText)
                      ? h(MlcText, { size: 12, lineHeight: 1.4, style: { color: text(runtimeProps.timeColor, palette.text) } }, () => text(item.timeText))
                      : null,
                    h("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px" } }, [
                      h(
                        MlcText,
                      ...
                      text(item.statusText)
                        ? h(MlcText, { size: 12, lineHeight: 1.4, style: { flex: "0 0 auto", color: text(runtimeProps.statusColor, palette.text) } }, () => text(item.statusText))
                        : null,
                    ]),
                    text(item.description)
                      ? h(MlcText, { as: "p", size: 13, lineHeight: 1.55, style: { color: text(runtimeProps.itemDescriptionColor, "#64748b") } }, () => text(item.description))
                ...
                return h(
                  ...
                  {
                    key: text(item.id, `${index}`),
                    class: `mlc-basic-timeline__item mlc-basic-timeline__item--${status}`,
                    style: { display: "grid", gridTemplateColumns: "auto 1fr", gap: "10px", alignItems: "stretch" } satisfies CSSProperties,
                  },
                  [
                    h(
                      "div",
                      {
                        class: "mlc-basic-timeline__rail",
                        style: {
                          display: "grid",
                          justifyItems: "center",
                          gridTemplateRows: "auto 1fr",
                          paddingTop: marker === "badge" ? "2px" : "8px",
                        } satisfies CSSProperties,
                      },
                      [
                        ...
                        showConnector && index < items.length - 1
                          ? h("span", {
                              class: "mlc-basic-timeline__connector",
                              "aria-hidden": "true",
                              style: {
                                width: `${number(runtimeProps.lineWidth, 2)}px`,
                                minHeight: `${Math.max(18, gap + number(runtimeProps.itemPadding, 12))}px`,
                                marginTop: "8px",
                                borderRadius: "999px",
                                background: text(runtimeProps.lineColor, palette.line),
                              } satisfies CSSProperties,
                            })
                          : null,
                      ],
                    ...
                    clickable
                      ? h(
                          ...
                          {
                            type: "button",
                            class: "mlc-basic-timeline__button",
                            style: {
                              display: "block",
                              width: "100%",
                              padding: 0,
...
export const BasicAlert = defineComponent({
  name: "BasicAlert",
  props: materialPropOptions,
  setup(props) {
    return () => {
      const runtimeProps = props.props ?? {};
      const tone = option<BasicAlertTone>(runtimeProps.tone, ["info", "success", "warning", "danger", "neutral"], "info");
      const variant = option<BasicAlertVariant>(runtimeProps.variant, ["soft", "outline", "solid"], "soft");
      const palette = BASIC_ALERT_TONE_PALETTES[tone];
      const accentColor = text(runtimeProps.accentColor, palette.accent);
      const solid = variant === "solid";
      const title = text(runtimeProps.title, "基础提示");
      const content = text(runtimeProps.content, "用于配置页面提示、注意事项或反馈说明。");
      const badgeText = text(runtimeProps.badgeText);
      const actionText = text(runtimeProps.actionText);
      const actionAlign = option<BasicInlineAlign>(runtimeProps.actionAlign, ["left", "center", "right"], "left");
      const showAction = boolean(runtimeProps.showAction, Boolean(actionText));
      const onActionClick = typeof runtimeProps.onActionClick === "function" ? runtimeProps.onActionClick as BasicAlertActionHandler : undefined;
      const backgroundColor = text(runtimeProps.backgroundColor, solid ? accentColor : palette.background);
      const borderColor = text(runtimeProps.borderColor, variant === "soft" ? palette.border : accentColor);
      const titleColor = text(runtimeProps.titleColor, solid ? "#ffffff" : palette.title);
      const contentColor = text(runtimeProps.contentColor, solid ? "rgba(255, 255, 255, 0.88)" : palette.content);

      return h(
        "section",
        {
          class: `mlc-material mlc-basic-alert mlc-basic-alert--${tone} mlc-basic-alert--${variant}`,
          style: {
            padding: `${number(runtimeProps.paddingY, 12)}px 12px`,
        ...
        h(
          "div",
          {
            class: "mlc-basic-alert__card",
            role: "note",
            style: {
              display: "grid",
              gridTemplateColumns: boolean(runtimeProps.showIcon, true) ? "auto 1fr" : "1fr",
              gap: `${number(runtimeProps.gap, 10)}px`,
              alignItems: "flex-start",
              padding: `${number(runtimeProps.padding, 14)}px`,
              borderRadius: `${number(runtimeProps.radius, 12)}px`,
              border: `${number(runtimeProps.borderWidth, 1)}px solid ${borderColor}`,
              color: titleColor,
          ...
          [
            boolean(runtimeProps.showIcon, true)
              ? h(
                  "span",
                  {
                    class: "mlc-basic-alert__icon",
                    "aria-hidden": "true",
                    style: {
                      display: "inline-flex",
                      alignItems: "center",
                      justifyContent: "center",
                      width: `${number(runtimeProps.iconSize, 24)}px`,
                      height: `${number(runtimeProps.iconSize, 24)}px`,
                      borderRadius: "999px",
                      color: text(runtimeProps.iconColor, solid ? accentColor : "#ffffff"),
                      background: text(runtimeProps.iconBackgroundColor, solid ? "#ffffff" : accentColor),
                      fontSize: `${Math.max(12, Math.round(number(runtimeProps.iconSize, 24) * 0.58))}px`,
                      fontWeight: 900,
                      lineHeight: 1,
                    } satisfies CSSProperties,
                  },
                  text(runtimeProps.iconText, palette.icon),
                )
              : null,
            h("div", { class: "mlc-basic-alert__content", style: { display: "grid", gap: "8px", minWidth: 0 } }, [
              h("div", { style: { display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px" } }, [
                h(
                  MlcText,
                  { as: "strong", size: number(runtimeProps.titleSize, 15), weight: 900, lineHeight: 1.45, style: { color: titleColor } },
                  () => title,
                ),
                badgeText
                  ? h(
                      MlcTag,
                      {
                        ...
                        radius: number(runtimeProps.badgeRadius, 999),
                        class: "mlc-basic-alert__badge",
                        style: {
                          color: text(runtimeProps.badgeColor, accentColor),
                          background: text(runtimeProps.badgeBackgroundColor, solid ? "#ffffff" : "rgba(255, 255, 255, 0.76)"),
                        },
                      },
                      () => badgeText,
                    )
              ...
              content
                ? h(
                    MlcText,
                    { as: "p", size: number(runtimeProps.contentSize, 13), lineHeight: 1.65, style: { color: contentColor, whiteSpace: "pre-line" } },
                    () => content,
                  )
                : null,
              showAction && actionText
                ? h(
                    "div",
                    {
                      class: "mlc-basic-alert__action",
                      style: {
                        display: "flex",
                        justifyContent: actionAlign === "right" ? "flex-end" : actionAlign === "center" ? "center" : "flex-start",
                      } satisfies CSSProperties,
                    },
                    h(
                      MlcButton,
                      {
                        ...
                        style: {
                          ...
                          color: solid ? "#ffffff" : text(runtimeProps.actionTextColor, "#ffffff"),
                          background: solid ? "transparent" : text(runtimeProps.actionBackgroundColor, accentColor),
                        },
                        onClick: () => {
                          onActionClick?.();
                        },
...
export const BasicMetric = defineComponent({
  name: "BasicMetric",
  props: materialPropOptions,
  setup(props) {
    return () => {
      const runtimeProps = props.props ?? {};
      const tone = option<BasicMetricTone>(runtimeProps.tone, ["brand", "success", "warning", "danger", "neutral"], "brand");
      const variant = option<BasicMetricVariant>(runtimeProps.variant, ["card", "plain"], "card");
      ...
      return h(
        "section",
        {
          class: `mlc-material mlc-basic-metric mlc-basic-metric--${tone} mlc-basic-metric--${variant}`,
          style: {
            padding: `${number(runtimeProps.paddingY, 14)}px 12px`,
            background: text(runtimeProps.wrapperBackgroundColor, "#f3f4f6"),
          } satisfies CSSProperties,
        ...
        h(
          "div",
          {
            class: "mlc-basic-metric__card",
            style: {
              display: "grid",
              padding: variant === "card" ? `${number(runtimeProps.padding, 14)}px` : 0,
              border: variant === "card" ? `${number(runtimeProps.borderWidth, 1)}px solid ${text(runtimeProps.borderColor, palette.border)}` : "0 solid transparent",
              borderRadius: variant === "card" ? `${number(runtimeProps.radius, 12)}px` : 0,
              background: variant === "card" ? text(runtimeProps.backgroundColor, palette.background) : "transparent",
              boxShadow: variant === "card" && boolean(runtimeProps.shadow) ? "0 10px 24px rgba(15, 23, 42, 0.08)" : "none",
            } satisfies CSSProperties,
          },
          h(MlcMetric, {
            label: text(runtimeProps.label, "基础指标"),
            value: text(runtimeProps.value, "1280"),
            prefix: text(runtimeProps.prefix),
            suffix: text(runtimeProps.suffix),
            helperText: text(runtimeProps.helperText, "用于展示静态数字指标或配置摘要。"),
            align,
            ...
            valueSize: number(runtimeProps.valueSize, 30),
            labelSize: number(runtimeProps.labelSize, 13),
...
export const BasicInput = defineComponent({
  ...
  props: materialPropOptions,
  setup(props) {
    const validation = injectBasicFormValidation();
    ...
    return () => {
      ...
      const fieldError = getBasicFormFieldError(validation, props.node);
      const fieldErrorColor = validation?.errorColor.value ?? BASIC_FORM_DEFAULT_ERROR_COLOR;

      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-input", fieldError),
          style: {
            display: "grid",
            gap: "8px",
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
        ...
        [
          label
            ...
            : null,
          h(MlcInput, {
            class: fieldError ? "mlc-basic-form__control--invalid" : undefined,
            value: value.value,
            type: inputType,
            ...
            radius: number(runtimeProps.radius, 8),
            style: {
              borderColor: fieldError ? fieldErrorColor : text(runtimeProps.borderColor, "#e5e7eb"),
              color: text(runtimeProps.textColor, "#111827"),
              background: text(runtimeProps.inputBackgroundColor, "#ffffff"),
            } satisfies CSSProperties,
            onChange: (nextValue: string) => {
              value.value = nextValue;
              clearBasicFormFieldError(validation, props.node);
...
export const BasicTextarea = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      const runtimeProps = props.props ?? {};
      const label = text(runtimeProps.label, "基础多行输入");
      const helperText = text(runtimeProps.helperText);
      const handler = runtimeProps.onChange;
      const fieldError = getBasicFormFieldError(validation, props.node);
      const fieldErrorColor = validation?.errorColor.value ?? BASIC_FORM_DEFAULT_ERROR_COLOR;
      ...
      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-textarea", fieldError),
          style: {
            display: "grid",
            gap: "8px",
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
        ...
        [
          ...
          h(MlcTextarea, {
            class: fieldError ? "mlc-basic-form__control--invalid" : undefined,
            value: value.value,
            rows: number(runtimeProps.rows, 3),
            placeholder: text(runtimeProps.placeholder, "请输入多行内容"),
            disabled: boolean(runtimeProps.disabled),
            radius: number(runtimeProps.radius, 8),
            style: {
              borderColor: fieldError ? fieldErrorColor : text(runtimeProps.borderColor, "#e5e7eb"),
              color: text(runtimeProps.textColor, "#111827"),
              background: text(runtimeProps.textareaBackgroundColor, "#ffffff"),
            } satisfies CSSProperties,
            onChange: (nextValue: string) => {
              value.value = nextValue;
              clearBasicFormFieldError(validation, props.node);
...
export const BasicSelect = defineComponent({
  ...
  setup(props) {
    ...
    return () => {
      const runtimeProps = props.props ?? {};
      const label = text(runtimeProps.label, "基础选择框");
      const helperText = text(runtimeProps.helperText);
      ...
      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-select", fieldError),
          style: {
            display: "grid",
            gap: "8px",
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
        ...
        [
          ...
          h(MlcSelect, {
            class: fieldError ? "mlc-basic-form__control--invalid" : undefined,
            value: value.value,
            placeholder: text(runtimeProps.placeholder, "请选择"),
            disabled: boolean(runtimeProps.disabled),
            radius: number(runtimeProps.radius, 8),
  ...
  setup(props) {
    ...
    return () => {
      ...
      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-radio-group", fieldError),
          style: {
            display: "grid",
            gap: "8px",
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
  ...
  setup(props) {
    ...
    return () => {
      ...
      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-stepper", fieldError),
          style: {
            display: "grid",
            gap: "8px",
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
...
export const BasicSwitch = defineComponent({
  name: "BasicSwitch",
  props: materialPropOptions,
  setup(props) {
    const validation = injectBasicFormValidation();
    ...
    return () => {
      const runtimeProps = props.props ?? {};
      const label = text(runtimeProps.label, "基础开关");
      const checkedText = text(runtimeProps.checkedText, "已开启");
      const uncheckedText = text(runtimeProps.uncheckedText, "已关闭");
      const helperText = text(runtimeProps.helperText);
      const handler = runtimeProps.onChange;
      const fieldError = getBasicFormFieldError(validation, props.node);
      const fieldErrorColor = validation?.errorColor.value ?? BASIC_FORM_DEFAULT_ERROR_COLOR;
      ...
      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-switch", fieldError),
          style: {
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
        ...
        [
          h(
            MlcSwitch,
            {
              ...
              disabled: boolean(runtimeProps.disabled),
              activeColor: text(runtimeProps.activeColor, "#0f766e"),
              inactiveColor: text(runtimeProps.inactiveColor, "#cbd5e1"),
              thumbColor: text(runtimeProps.thumbColor, "#ffffff"),
              ...
              h("span", { style: { display: "grid", gap: "2px" } }, [
                ...
                h(
                  MlcText,
                  {
                    size: 12,
                    tone: "muted",
                    style: { color: text(runtimeProps.stateTextColor, "#64748b") } satisfies CSSProperties,
                  },
                  () => (checked.value ? checkedText : uncheckedText),
                ),
              ]),
          ),
          createBasicFormHiddenField(
  ...
  setup(props) {
    ...
    return () => {
      ...
      return h(
        "section",
        {
          ...createBasicFormFieldAttrs("mlc-material mlc-basic-checkbox", fieldError),
          style: {
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
            color: text(runtimeProps.labelColor, "#111827"),
            background: text(runtimeProps.wrapperBackgroundColor, "transparent"),
...
export const BasicVideo = defineComponent({
  name: "BasicVideo",
  props: materialPropOptions,
  setup(props) {
    return () => {
      const runtimeProps = props.props ?? {};
      const videoUrl = text(runtimeProps.videoUrl);
      const posterUrl = text(runtimeProps.posterUrl);
      const radius = number(runtimeProps.radius, 12);
      const onPlay = runtimeProps.onPlay;

      return h(
        "section",
        {
          class: "mlc-material mlc-basic-video",
          style: {
            padding: `${number(runtimeProps.paddingY, 12)}px 16px`,
            background: text(runtimeProps.wrapperBackgroundColor, "transparent"),
          } satisfies CSSProperties,
        ...
        [
          h(
            "article",
            {
              class: "mlc-basic-video__card",
              style: {
                overflow: "hidden",
                borderRadius: `${radius}px`,
                background: text(runtimeProps.cardBackgroundColor, "#ffffff"),
                border: `1px solid ${text(runtimeProps.borderColor, "#e5e7eb")}`,
                boxShadow: boolean(runtimeProps.shadow, true) ? "0 12px 30px rgba(15, 23, 42, 0.12)" : undefined,
              } satisfies CSSProperties,
            },
            [
              videoUrl
                ? h("video", {
                    class: "mlc-basic-video__media",
                    src: videoUrl,
                    poster: posterUrl || undefined,
                    controls: boolean(runtimeProps.controls, true),
                    autoplay: boolean(runtimeProps.autoPlay),
                    muted: boolean(runtimeProps.muted, true),
                    loop: boolean(runtimeProps.loop),
                    playsinline: boolean(runtimeProps.playsInline, true),
                    preload: "metadata",
                    style: {
                      display: "block",
                      width: "100%",
                      aspectRatio: text(runtimeProps.ratio, "16 / 9"),
                      objectFit: "cover",
                      background: text(runtimeProps.mediaBackgroundColor, "#0f172a"),
                    } satisfies CSSProperties,
                    onPlay: () => {
                ...
                : h(
                    ...
                    [
                      h(MlcImage, {
                        src: posterUrl,
                        alt: text(runtimeProps.title, "基础视频"),
                        ratio: text(runtimeProps.ratio, "16 / 9"),
                        fit: "cover",
                        radius: 0,
                        fallback: "请配置视频地址",
              ...
              h(
                ...
                [
                  ...
                  h(
                    ...
                    () => text(runtimeProps.title, "基础视频"),
                  ),
                  text(runtimeProps.description)
                    ? h(
                        MlcText,
                        {
                          as: "p",
...
export const GridContainer = defineComponent({
  ...
  setup(props, { slots }) {
    return () => {
      ...
      const subtitle = text(runtimeProps.subtitle);
      const children = slots.default?.();
      const columns = Math.min(3, Math.max(2, Math.round(number(runtimeProps.columns, 2))));
      const gap = number(runtimeProps.gap, 10);
      const marginY = number(runtimeProps.marginY, 10);
      const borderWidth = number(runtimeProps.borderWidth, 0);
      const radius = number(runtimeProps.radius, 10);
      ...
      return h(
        "section",
        {
          class: "mlc-material mlc-grid-container",
          style: {
            margin: `${marginY}px 0`,
            padding: `${number(runtimeProps.padding, 12)}px`,
            background: text(runtimeProps.backgroundColor, "#ffffff"),
        ...
        [
          title
            ? h(
                MlcText,
                {
                  ...
                  weight: 800,
                  lineHeight: 1.35,
                  class: "mlc-grid-container__title",
                  style: { margin: "0 0 6px", color: text(runtimeProps.titleColor, "#111827") },
                },
                () => title,
              )
          ...
          subtitle
            ? h(
                MlcText,
                {
                  ...
                  size: 13,
                  lineHeight: 1.6,
                  class: "mlc-grid-container__subtitle",
                  style: { margin: "0 0 12px", color: text(runtimeProps.subtitleColor, "#64748b") },
                },
                () => subtitle,
              )
          ...
          children?.length
            ? h(
                "div",
                {
                  class: "mlc-grid-container__body",
                  style: {
                    display: "grid",
                    gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`,
                    gap: `${gap}px`,
                ...
                children.map((child) => h("div", { class: "mlc-grid-container__cell", style: { minWidth: 0 } satisfies CSSProperties }, [child])),
              )
            : h(
                MlcText,
                {
                  ...
                  tone: "muted",
                  lineHeight: 1.5,
                  class: "mlc-grid-container__empty",
                },
                () => text(runtimeProps.emptyText, "向网格容器中添加物料"),
              ),
        ],
      );
    };
...
export const h5VueMaterials: LowcodeMaterial<VueH5MaterialComponent>[] = [
  ...
  {
    component: BasicVideo as VueH5MaterialComponent,
    manifest: createMaterialManifest({
