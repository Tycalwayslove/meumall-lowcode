...
`BasicTimeline` 是通用静态时间线物料，负责标题、说明、节点标题、节点说明、时间文案、状态视觉、标记样式、连接线、间距、边框、圆角、颜色和 `onItemClick` 安全事件；它不承载远程活动进度、订单状态、审批流程、任务流、服务履约、商品/优惠券/门店达人等业务状态模型或个性化推荐语义。这些能力应由后续数据源协议、业务物料、管理台工作流或宿主服务扩展。
...
`BasicMetric` 是通用静态指标物料，负责指标标签、数值、前缀、后缀、说明、语气、对齐、卡片样式、颜色、圆角、留白和阴影配置；它不承载远程统计、实时刷新、库存计算、销量计算、人数计算、埋点聚合或服务端格式化。这些能力应由后续数据源协议、业务物料、Java/BFF 或宿主服务扩展。

`BasicMetricGrid` 是通用静态指标组物料，负责标题、说明、多项指标数组、列数、语气、对齐、卡片/简洁样式、颜色、圆角、留白和阴影配置；它继续复用 `MlcMetric`，不承载远程统计、实时刷新、库存计算、销量计算、人数计算、埋点聚合或服务端格式化。这些能力应由后续数据源协议、业务物料、Java/BFF 或宿主服务扩展。
